...
// Extrae userId del JWT sin librería extra
...
function ActiveChat() {
  ...
  // Suscripciones WS para la conversación activa
  useWebSocket({
    conversationId: activeConversation?.conversationId ?? null,
    onMessage: (msg) => appendMessage(msg),
    onTyping: (evt) => {
      if (evt.senderId !== currentUserId) {
        setTypingUserId(evt.typing ? evt.senderId : null);
        if (evt.typing) setTimeout(() => setTypingUserId(null), 3_000);
      }
    },
  });

  // Scroll al fondo en mensajes nuevos
  useEffect(() => {
