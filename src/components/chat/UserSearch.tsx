...
export function UserSearch({ onConversationReady }: Props) {
  ...
  return (
    <div ref={wrapperRef} className="relative px-3 py-2 border-b border-gray-100">
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-lg border border-gray-200 bg-gray-50 py-2 pl-9 pr-8 text-sm outline-none transition focus:border-blue-400 focus:bg-white focus:ring-2 focus:ring-blue-100"
        />
        {isSearching && (
              ...
              {results.map((user) => (
                <li key={user.id}>
                  <button
                    ...
                    className="flex w-full items-center gap-3 px-4 py-2.5 text-left hover:bg-gray-50 transition disabled:opacity-50"
                  >
